...
<div class="content">
                ...
                <?php if (isset($reponses) && count($reponses) > 0): ?>
                    <?php foreach ($reponses as $reponse): ?>
                        <tr>
                            ...
                            <button class="button" onclick="location.href='index3.php?action=deleteReponse&id=<?= $reponse['id_reponse']; ?>'">
    ...
    <div class="pagination">
        <?php for ($i = 1; $i <= $totalPages; $i++): ?>
            <a href="index3.php?action=indexr&page=<?= $i ?>&search=<?= urlencode($search) ?>" class="<?= $i == $page ? 'active' : ''; ?>"><?= $i ?></a>
        <?php endfor; ?>
    </div>
</div>
